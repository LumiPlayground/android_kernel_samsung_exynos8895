		...
		struct device_attribute *attr, char *buf)
{
	...
	len += snprintf(&buf[len], PAGE_SIZE,"%d,", temp[0]);
	len += snprintf(&buf[len], PAGE_SIZE,"%d,", temp[1]);
	len += snprintf(&buf[len], PAGE_SIZE,"%d,", temp[2]);
	len += snprintf(&buf[len], PAGE_SIZE,"%d", temp[3]);
