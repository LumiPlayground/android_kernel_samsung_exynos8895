...
static int __init tcrypt_mod_init(void)
{
	...
	if (err) {
		printk(KERN_INFO "FIPS : POST - one or more algorithm tests failed\n");
		set_in_fips_err();
		goto err_free_tv;
	} else {
		printk(KERN_INFO "FIPS : POST - Algorithm Tests Passed\n");
		if (do_integrity_check() != 0)
		{
#ifndef CONFIG_FUNCTION_TRACER
			printk(KERN_INFO "FIPS : POST - Integrity Check Failed\n");
			set_in_fips_err();
#else
			printk(KERN_ERR "FIPS : POST - Integrity Check bypassed due to ftrace debug mode\n");
#endif
		}
		else
		{
			printk(KERN_INFO "FIPS : POST - Integrity Check Passed\n");
		}
		if(in_fips_err()) {
			printk(KERN_INFO "FIPS : POST - CRYPTO API in FIPS Error\n");
		} else {
			printk(KERN_INFO "FIPS : POST - CRYPTO API started in FIPS approved mode\n");
		}
	}
